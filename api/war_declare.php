...
// Требуется отправить хотя бы одного бойца. Отправка нуля войск недопустима.
$total = $peh + $kaz + $gva;
if ($total <= 0) {
...
// проверяем, что есть нужное количество войск у атакующего на основе army_state.
// Получаем локальное состояние армии (infantry, cossacks, guards)
$stArmyCheck = $pdo->prepare('SELECT infantry, cossacks, guards FROM army_state WHERE ruler_name = ? LIMIT 1');
$stArmyCheck->execute([$attacker]);
$attArmyRow = $stArmyCheck->fetch();
if (!$attArmyRow) {
    respond(false, 'Армия атакующего не найдена');
}
if ($peh > (int)$attArmyRow['infantry'] ||
    $kaz > (int)$attArmyRow['cossacks'] ||
    $gva > (int)$attArmyRow['guards']) {
    respond(false, 'Недостаточно войск для отправки');
}

// списываем войска из army_state для атакующего
$updArmyState = $pdo->prepare('
    UPDATE army_state
    SET infantry = infantry - ?, cossacks = cossacks - ?, guards = guards - ?
    WHERE ruler_name = ?
');
$updArmyState->execute([$peh, $kaz, $gva, $attacker]);

// также синхронизируем таблицу gos_app, чтобы gov‑приложение отражало актуальное состояние армии.
// После списания войск в army_state, считываем текущие остатки и перезаписываем в gos_app.
$stNewState = $pdo->prepare('SELECT infantry, cossacks, guards FROM army_state WHERE ruler_name = ? LIMIT 1');
$stNewState->execute([$attacker]);
$newArmyState = $stNewState->fetch();
if ($newArmyState) {
    $updArmy = $pdo->prepare('
        UPDATE gos_app 
        SET peh = ?, kaz = ?, gva = ?
    ...
    ');
    $updArmy->execute([
        (int)$newArmyState['infantry'],
        (int)$newArmyState['cossacks'],
        (int)$newArmyState['guards'],
        $attacker
    ]);
}
