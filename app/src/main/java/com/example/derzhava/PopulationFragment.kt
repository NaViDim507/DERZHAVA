...
class PopulationFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val country = db.countryDao().getCountryByRuler(user.rulerName)
        if (country == null) {
            Toast.makeText(requireContext(), "Страна не найдена", Toast.LENGTH_SHORT).show()
            return
        }

        // Население делится на свободных рабочих, работающих на добыче, учёных и армию.
        ...
        val army = db.armyDao().getByRuler(user.rulerName)
    ...
    override fun onResume() {
        super.onResume()
        // При возврате на экран населения обновляем данные из сервера,
        // чтобы отразить любые изменения, сделанные в базе данных. Обновление
        // выполняется в фоновом потоке, затем UI перерисовывается.
        val user = userRepository.getUser() ?: return
        viewLifecycleOwner.lifecycleScope.launch {
            try {
                // синхронизируем страну и армию
                withContext(Dispatchers.IO) {
                    // Для страны используем syncDownOrCreate(), т.к. syncDown() отсутствует в OnlineCountrySync
