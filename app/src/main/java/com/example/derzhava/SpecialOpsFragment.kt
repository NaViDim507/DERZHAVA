...
class SpecialOpsFragment : Fragment() {
    ...
    private lateinit var generalDao: com.example.derzhava.data.GeneralDao
    ...
    private fun performSabotage(target: CountryEntity) {
        ...
        // Отправляем сообщение о результате диверсии
        run {
            val user = userRepository.getUser()
            if (user != null) {
    ...
    private fun performTheft(target: CountryEntity) {
        ...
        // Отправляем сообщение о результате воровства
        run {
            val user = userRepository.getUser()
            if (user != null) {
                val nowMsg = System.currentTimeMillis()
        ...
        androidx.appcompat.app.AlertDialog.Builder(ctx)
            ...
            .setPositiveButton("Вывести войска") { _, _ ->
                ...
                viewLifecycleOwner.lifecycleScope.launch {
                    try {
                        // Перед объявлением войны убедимся, что у нас есть генерал. Если генерала нет или
                        // его характеристики равны нулю, нельзя объявлять войну.
                        val hasGeneral = withContext(Dispatchers.IO) {
                            val g = generalDao.getByRuler(myRuler)
                            g != null && (g.attack + g.defense + g.leadership > 0)
                        }
                        if (!hasGeneral) {
                            Toast.makeText(requireContext(), "Сначала наймите генерала в Командном центре", Toast.LENGTH_LONG).show()
                            return@launch
                        }
                        ...
                        // 3. Зеркалим движение войск в локальный лог и создаём
                        // локальную запись войны, чтобы главная страница сразу
                        // отобразила войну до следующего запроса war_list.php.
                        val now = System.currentTimeMillis()
                        val warId = resp.war_id ?: now
                        warMoveDao.insert(
                            WarMoveEntity(
                                warId = warId,
                                type = "reinforce",
                        ...
                        warDao.insert(
                            WarEntity(
                                ...
                                attackerRuler = myRuler,
                                defenderRuler = enemyCountry.rulerName,
                                attackerCountry = myC.countryName,
                                defenderCountry = enemyCountry.countryName,
                                peh = peh,
                                kaz = kaz,
                                gva = gva,
                                catapults = effectiveKat,
                                startAt = now,
                        ...
                        bindTargets()
                        // Отправляем системное сообщение себе и противнику о начале войны
                        run {
                            val nowMsg = System.currentTimeMillis()
                            val attackerMsg = "Вы объявили войну ${enemyCountry.countryName}."
                            messageDao.insert(
                                MessageEntity(
                                    rulerName = myRuler,
                                    text = attackerMsg,
                            ...
                            val defenderMsg = "${myC.countryName} объявило вам войну."
                            messageDao.insert(
                                MessageEntity(
                                    rulerName = enemyCountry.rulerName,
                                    text = defenderMsg,
                                    timestampMillis = nowMsg,
                                    isRead = false
                                )
