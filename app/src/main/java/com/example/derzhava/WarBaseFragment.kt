...
class WarBaseFragment : Fragment() {
    ...
    override fun onAttach(context: Context) {
        ...
        // Инициализируем DAO для сообщений
    ...
    private fun processTrainingJob() {
        // Локальная страна и армия для обновления
        val c = country ?: return
        val a = army ?: return

        // Сохраняем текущую локальную задачу, если есть
        val localJob = trainingJob
        // Загружаем актуальное задание с сервера. Метод trainingJobDao.getJobForRuler
        // возвращает null, если задание уже завершено и удалено сервером.
        val remoteJob = trainingJobDao.getJobForRuler(c.rulerName)

        // Если сервер не вернул задание, но локальная задача ещё есть,
        // проверяем, прошло ли время обучения. Если да, то возвращаем
        // учёных и добавляем обученных бойцов локально. Это решает
        // проблему, когда сервер завершил задание, но страна ещё не
        // синхронизирована — учёные могли "пропасть" до следующего syncDown().
        if (remoteJob == null) {
            if (localJob != null) {
                val endTimeMillis = localJob.startTimeMillis + localJob.durationSeconds * 1000L
                val now = System.currentTimeMillis()
                if (now >= endTimeMillis) {
                    // Возвращаем учёных и добавляем обученных бойцов
                    val newBots = c.bots + localJob.scientists
                    val newArmy = when (localJob.unitType) {
                        1 -> a.copy(infantry = a.infantry + localJob.workers)
                        2 -> a.copy(cossacks = a.cossacks + localJob.workers)
                        3 -> a.copy(guards = a.guards + localJob.workers)
                        else -> a
                    }
            ...
            // Очищаем локальное задание
            trainingJob = null
            renderTrainingStatus(null)
            return
        }

        // Если задание всё ещё существует на сервере, обновляем локальную ссылку
        trainingJob = remoteJob
        val endTimeMillis = remoteJob.startTimeMillis + remoteJob.durationSeconds * 1000L
        val now = System.currentTimeMillis()
        if (now >= endTimeMillis) {
            // Обучение завершилось. Сервер уже добавил войска и вернул учёных.
            // Тем не менее, нам нужно обновить локальные сущности, чтобы они
            // отражали актуальное состояние до следующей синхронизации.
            val newBots = c.bots + remoteJob.scientists
            val newArmy = when (remoteJob.unitType) {
                1 -> a.copy(infantry = a.infantry + remoteJob.workers)
                2 -> a.copy(cossacks = a.cossacks + remoteJob.workers)
                3 -> a.copy(guards = a.guards + remoteJob.workers)
                else -> a
            }
            val newCountry = c.copy(bots = newBots)
            ...
            // Обновляем статус оставшегося времени обучения
    ...
    private fun showTrainTroopsDialog() {
        ...
        AlertDialog.Builder(ctx)
            ...
            .setPositiveButton("Начать") { _, _ ->
                ...
                // Отправляем системное сообщение о начале обучения войск
                run {
                    val nowMsg = System.currentTimeMillis()
                    val unitNameMsg = when (unitType) {
                        1 -> "пехоты"
                        2 -> "казаков"
                        3 -> "гвардии"
                        else -> "войск"
                    ...
                    val msgText =
                        "Начато обучение $rob $unitNameMsg. Завершится через ${e4 / 60} мин."
                    messageDao.insert(
                        MessageEntity(
                            rulerName = c.rulerName,
                            text = msgText,
