...
class GameFragment : Fragment() {
    ...
    override fun onResume() {
        super.onResume()
        // Сохраняем цвет кнопки сообщений, если ещё не сохранён
        if (defaultMessagesTextColor == null) {
            defaultMessagesTextColor = binding.btnRefresh.currentTextColor
        }
    ...
    private fun setupClicks(country: CountryEntity) = with(binding) {
        ...
        btnProfileTop.setOnClickListener {
            btnProfileTop.setOnClickListener {
                // раньше был toast "Профиль..."
                // теперь просто перезагружаем главный экран
                (activity as? MainActivity)?.openGameScreen()
            }
        }
        btnStats.setOnClickListener {
            toast("Общая статистика державы пока в разработке")
        }
        ...
        btnLeaders.setOnClickListener {
            toast("Лидеры игры в разработке")
        }
