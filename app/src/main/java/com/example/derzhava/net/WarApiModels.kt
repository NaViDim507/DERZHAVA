...
data class WarDto(
    ...
    @SerializedName("attacker_won") val attackerWon: Boolean?,
    val reconAcc: Int
)
...
// Ответ сервера на попытку обороны. Помимо флага success и сообщения может возвращать
// дополнительные данные о новом состоянии войны и остатках войск атакующего. В случае
// победы обороны поле attackerWon будет равно 0, и война считается завершённой.
data class WarDefenseResponse(
    val success: Boolean,
    val message: String?,
    val state: String? = null,
    @SerializedName("attacker_peh") val attackerPeh: Int? = null,
    @SerializedName("attacker_kaz") val attackerKaz: Int? = null,
    @SerializedName("attacker_gva") val attackerGva: Int? = null,
    @SerializedName("attacker_won") val attackerWon: Int? = null,
    @SerializedName("war_id") val warId: Long? = null
)
